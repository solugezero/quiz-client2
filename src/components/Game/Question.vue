...
import { gameGetQuestionAnswers } from '~/api';
    ...
    const startResultCount = () => {
        showSuperGameEnd.value = false; showCountingModal.value = true;
        setTimeout(() => {
    ...
    const stickerDrag = (i, ev) => {
        ...
        positionAnswers.value[i] = item;
        if (positionAnswers.value[0] !== null && positionAnswers.value[0] !== undefined && positionAnswers.value[1] !== null && positionAnswers.value[1] !== undefined && positionAnswers.value[2] !== null && positionAnswers.value[2] !== undefined) {
  ...
  const answerEmit = async (answer) => {
    ...
    /* watch(
        ...
        () => {
			...
			if (totalGameTime.value - props.time <= 0) {
...
<template>
    ...
    <div v-if="question.meta && !question.meta.easy" class="flex items-center gap-x-9px scale-90 sm:hidden" :class="{'pointer-events-none!': answerEmitting }">
        <div class="timer bg-white lt-xxxl:h-55px! lt-xxxl:max-w-310px q-shadow-sm lt-xxxl:p-10px!">
          ...
          <!-- TODO: more font -->
          <p class="text-22px lt-xxxl:text-16px w-35px lt-xxxl:w-20px text-end" v-if="question.type !== 'VALUE'">{{totalTime - time}}</p>
          <p class="text-22px lt-xxxl:text-16px w-35px lt-xxxl:w-20px text-end" v-else>{{totalGameTime - time}}</p>
        </div>
    ...
    <div v-if="question.meta && question.meta.easy" class="flex items-center gap-x-9px scale-90 sm:hidden w-full" :class="{'pointer-events-none!': answerEmitting }">
    ...
    <div class="card p-42px lt-xxxl:p-33px pb-48px w-770px lt-xxxl:w-610px lt-xxl:scale-90 lg:scale-100! max-h-[calc(100vh-50px)] scroll-y max-w-full lt-sm:-mt-20px" :class="[{ 'w-1120px! max-w-1120px! lt-lg:max-w-700px! lt-sm:max-w-full!': question.type === 'POSITION' }, { 'q-shadow-md__light-green': question.type === 'VALUE' }, { 'q-shadow-md': question.type !== 'VALUE' }, {'pointer-events-none!': answerEmitting }]">
      <div v-if="question.meta && !question.meta.easy" class="flex items-center gap-x-9px lt-sm:hidden">
        <div class="timer bg-white lt-xxxl:h-55px! lt-xxxl:max-w-310px q-shadow-sm lt-xxxl:p-10px!">
          <img class="w-24px h-24px lt-xxxl:w-18px lt-xxxl:h-18px" src="/img/clock.png" alt="таймер">
          <div class="time lt-xxxl:h-13px! lt-xxxl:w-220px! lt-xxxl:min-w-220px! bg-#D1EAD8!">
            <div v-if="question.type !== 'VALUE'" class="inner h-full bg-#71CB8A trs" :style="`width: ${100 - ((100 * time) / totalTime)}%`"></div>
        ...
        <div v-if="question.meta && question.meta.easy" class="flex items-center gap-x-9px lt-sm:hidden">
            <div @click="goToStart" class="timer bg-white lt-xxxl:h-55px! q-shadow-sm p-25px! max-w-98px! h-82px! cursor-pointer">
                <img src="/img/logo.svg" class="w-46px h-30px">
            </div>
            <div class="flex-1"></div>
            <div class="bg-white w-98px lt-xxxl:w-76px lt-xxxl:h-55px! py-12px border-2px border-#C9C9C9 flex flex-col items-center justify-center q-shadow-sm">
      ...
      <template v-if="question.meta">
        <div class="flex flex-col mt-42px mb-38px lt-xxxl:mb-22px lt-sm:mt-0!">
            ...
            <p v-html="question.meta.title" class="text-40px lt-xxxl:text-28px font-700 leading-130% mt-16px"></p>
            <div class="relative">
                <img v-if="question.meta.image && question.type !== 'POSITION'" class="w-full h-256px lt-xxxl:h-200px object-cover q-shadow-sm mt-24px" :src="`https://api.рускод.рф/${question.meta.image}`" >
                <p class="absolute bottom-14px lt-sm:left-8px lt-sm:bottom-8px lt-sm:right-unset right-14px text-white text-14px lt-sm:text-8px leading-130%" v-if="question.meta.imageComment">{{ question.meta.imageComment }}</p>
            </div>
            <p v-if="question.meta.comment" v-html="question.meta.comment" class="font-500 text-26px lt-xxxl:text-22px leading-130% text-#1E2947 mt-42px lt-xxxl:mt-32px"></p>
            <p v-else class="font-500 text-26px lt-xxxl:text-22px leading-130% text-#1E2947 mt-28px lg:hidden">Выберите ответ</p>
        </div> 
        <!-- TODO: переносы -->
        <div v-if="question.type === 'DEFAULT'" class="flex flex-wrap gap-16px" :class="{ 'mt-22px!': question.meta && question.meta.comment }">
            <div v-for="answer in question.answers" @click="answerEmit(answer)"
                ...
                <p v-html="answer.meta.title" class="text-20px lt-xxxl:text-14px leading-130% text-center py-22px p-18px"></p>
            </div>
        </div>
        <div v-else-if="question.type === 'VALUE'" class="">
            ...
            <div class="flex items-start gap-12px mt-10px lt-sm:flex-wrap">
                <div class="flex flex-col items-center gap-y-8px" v-for="letter, index in question.meta.value">
                    <p v-if="!question.meta.easy" class="font-500 text-26px lt-xxxl:text-22px leading-34px ">{{ letter.question }}</p>
                    <div class="h-66px lt-xxxl:h-55px w-66px lt-xxxl:w-55px flex items-center border-1px border-dashed border-#1E2947 text-#1E2947"
                        :class="[{ 'correct q-shadow-sm__green': revealLetters && letterModel[index] === letter.answer}, { 'wrong q-shadow-sm__red': revealLetters && letterModel[index] !== letter.answer}]"
                    >
                        <input class="h-full w-full text-center text-26px lt-xxxl:text-22px font-100 placeholder:text-#787F91 uppercase hover:placeholder:text-#fff focus:placeholder:text-#fff" maxlength="1" v-model="letterModel[index]" placeholder="+" @input="letterHandler($event, index)" >
                    </div>
                </div>
            </div>
            <template v-if="question.meta && !question.meta.easy">
        ...
        <div v-else-if="question.type === 'POSITION'" class="flex flex-col lt-lg:flex-row flex-wrap lt-lg:gap-x-8px lt-sm:w-full">
            <div class="flex flex-wrap gap-16px lt-lg:flex-col lt-lg:w-[calc(100%/2-4px)]">
                ...
                >
                    <p class="text-20px lt-xxxl:text-18px lt-sm:text-14px leading-26px text-center py-22px p-18px lt-sm:px-12px my-auto w-full pointer-events-none">{{ position.title }}</p>
                </div>
            </div>
            <div class="flex flex-wrap gap-16px mt-18px lt-lg:mt-0 lt-lg:flex-col lt-lg:w-[calc(100%/2-4px)]">
                ...
                >
                    <p class="text-20px lt-xxxl:text-18px leading-23px text-center pointer-events-none">{{ positionAnswers[zone - 1] ? positionAnswers[zone - 1].meta.title : '' }}</p>
                </div>
            </div>
            <div class="flex flex-wrap gap-16px lt-lg:gap-6px mt-18px w-full">
                ...
                >
                    <div
                        class="flex items-center justify-center py-22px p-18px pointer-events-none"
                    >
        ...
        <TheModalBottom v-model="showSuperGameEnd">
            <div class="absolute bottom-0 left-50% translate-x-[-50%]">
                ...
                <div class="flex lt-sm:flex-col h-full lt-lg:max-h-166px! lt-sm:max-h-310px! lt-lg:min-h-166px items-center justify-between p-55px lt-lg:p-36px! gap-x-44px w-886px max-w-screen! lt-lg:w-672px min-h-223px relative z-2" :class="[{ 'bg-#D1EAD8': !superGameError }, { 'bg-#FFEAE7': superGameError }]">
                    <div class="">
                        <p class="text-#08882C font-500 text-36px lt-lg:text-30px leading-130%" :class="{ 'text-#C01E00': superGameError}">{{ !superGameError ? 'Так держать!' : 'Ай-ай-ай...' }}</p>
                        <p v-if="question.meta && question.meta.comment" class="text-26px leading-130% mt-18px lt-lg:text-18px lt-lg:mt-11px">{{ question.meta.comment }}</p>
                    </div>
                    <button @click="startResultCount" class="p-22px w-332px lt-xxxl:w-270px lt-sm:w-186px lt-sm:p-10px lt-xxxl:p-16px text-center lt-sm:mt-24px lt-sm:max-w-178px lt-sm:h-40px flex items-center" :class="[{ 'bg-white q-shadow-sm__light-green text-#08882C border-1px border-#08882C' : !superGameError }, { 'bg-#FFEAE7! q-shadow-sm__red text-#C01E00 border-1px border-#C01E00': superGameError}]">
        ...
        <TheModalBottom v-model="showCountingModal">
            <div class="flex w-screen justify-center items-end"> 
                ...
                <div class="relative bg-white p-42px lt-lg:p-24px w-760px lt-xxxl:w-568px lt-lg:max-w-500px min-h-420px lt-lg:min-h-350px q-shadow-md__light-green mb-300px -ml-150px lt-lg:ml-[-280px] lt-lg:mb-450px lt-sm:scale-85 lt-sm:mb-300px lt-sm:min-h-fit">
                    <p class="font-500 text-36px lt-lg:text-30px lt-xxxl:text-28px lt-sm:text-20px leading-130%">Пару минут, мы считаем результаты<span class="lg:hidden">, а пока почитай, для чего мы сделали этот квиз</span></p>
                ...
                <a href="https://kantiana.ru" target="_blank">
                    <img src="/img/bfu-white.svg" class="fixed bottom-40px left-50% translate-x-[-50%] color-white z-100" >
                </a>
            </div>
        </TheModalBottom>
    </div>
...
.count-time {
    ...
    .inner {
        ...
        animation: loading 20s cubic-bezier(0.455, 0.030, 0.515, 0.955) both;   
    }
}
