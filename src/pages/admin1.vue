...
const { featchAnswers, featchQuestions, getQuestionsHelper } =
  toRefs(adminStore);
...
<template>
  ...
  <div ma>
    <div class="grid xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-2">
		<div v-for="(question, idx) in getQuestionsHelper">
      <AdminQuestionCard :question="question" :key="idx" />
    </div>
    </div>
  </div>
</div>
